...
### Step 0: 图片内容分析
上传产品参考图后，系统将自动进行图片内容分析，提取关键信息和广告词语，为后续决策提供依据。

### Step 1: 痛点诊断
根据用户输入和图片分析结果，识别以下核心痛点类型：
...
```
# 首先根据图片内容分析结果匹配玩法
IF 图片内容包含 "商超场景" OR 图片风格 == "高端品质":
    推荐玩法 = 01牌子货-商超场景
    
ELSE IF 图片内容包含 "使用场景" OR 图片风格 == "故事性":
    推荐玩法 = 02 25宫格分镜叙事
    
ELSE IF 图片内容包含 "质检过程" OR 图片风格 == "专业严谨":
    推荐玩法 = 03质检发货现场
    
ELSE IF 图片内容包含 "微观效果" OR 图片风格 == "科技感":
    推荐玩法 = 04个护特效-微观视角
    
ELSE IF 图片内容包含 "专家推荐" OR 图片风格 == "权威专业":
    推荐玩法 = 05专家访谈-权威背书
    
ELSE IF 图片内容包含 "生长过程" OR 图片风格 == "效果可视化":
    推荐玩法 = 06植物/生物生长-效果可视化
    
ELSE IF 图片内容包含 "促销场景" OR 图片风格 == "紧迫感":
    推荐玩法 = 07假新闻报道-制造紧迫感
    
ELSE IF 图片内容包含 "细节质感" OR 图片风格 == "奢侈品质":
    推荐玩法 = 08探针镜头-奢侈品质感
    
ELSE IF 图片内容包含 "第一人称视角" OR 图片风格 == "沉浸式体验":
    推荐玩法 = 09 FPV第一人称开箱
    
ELSE IF 图片内容包含 "网红推荐" OR 图片风格 == "UGC质感":
    推荐玩法 = 10红人UGC量产
    
ELSE IF 图片内容包含 "分镜时间线" OR 图片风格 == "精确控制":
    推荐玩法 = 11 25宫格TikTok分镜
    
# 其次根据用户输入匹配玩法
ELSE IF 产品类型 == "白牌/无品牌" OR 用户反馈 == "看起来像廉价货":
    推荐玩法 = 01牌子货-商超场景
...
<image_analysis>
### 1. 图片元素识别
- **产品主体**：识别图片中的核心产品，包括产品类型、颜色、形状、材质等
- **背景环境**：识别图片中的背景场景，如商超、实验室、户外等
- **人物形象**：识别图片中的人物，包括性别、年龄、表情、动作等
- **文字信息**：识别图片中的文字内容，如品牌Logo、产品名称、卖点等

### 2. 图片风格分析
- **色彩风格**：分析图片的主色调、色彩搭配、饱和度等
- **构图风格**：分析图片的构图方式、视角、景深等
- **光影风格**：分析图片的光线方向、强度、阴影等
- **情感风格**：分析图片传达的情感、氛围、调性等

### 3. 广告词语提取
- **产品卖点**：从图片中提取产品的功能特性、外观特点、材质特性、适用场景等
- **品牌调性**：从图片中提取品牌的形象特点、故事元素、价值观等

### 4. 根据图片内容选择最合适的广告方式
- **玩法匹配**：根据图片内容匹配最适合的带货玩法
- **词语匹配**：根据图片内容匹配最适合的广告词语
- **调性匹配**：根据图片内容匹配最适合的品牌调性
</image_analysis>
...
<prompt_template>
**基础参数**：5-8s竖版视频，Seedance2.0fast模式，全能参考
**镜头语言**：[随意运镜/缓慢扫过货架]，[推进展示价格牌/拿起产品细看]
**核心主体**：@Image {product_image}的产品，整齐摆放在货架上，周围是同类竞品
**商业场景**：[美式大型仓储超市/高端商场专柜]，[超市特有的冷白光/商场暖光]，[轻微的价格标签/促销标识]
**强制约束**：画面中间浮现字幕"TikTok Shop限时优惠"，强调价格优势，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：10-15s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：按分镜顺序自动运镜，保持叙事连贯性
**核心主体**：@Image 25宫格分镜图，从左到右、从上到下
**商业场景**：每个格子对应一个场景：第一行[问题/痛点]，第二行[产品出现/开始使用]，第三行[使用过程/效果显现]，第四行[问题解决/效果完成]，第五行[用户满意/推荐]
**强制约束**：镜头间过渡自然，保持叙事连贯性，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：8-10s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：固定镜头，展示工人检查和打包过程
**核心主体**：@Image {product_image}的产品，工人仔细检查外观，[测试按键功能/检查缝线质量/确认无瑕疵]
**商业场景**：工厂发货现场，[防震泡沫/气柱袋/精美包装盒]，[快递单/美国地址标签]
**强制约束**：整个过程专业严谨，展现品质保障，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：8-10s竖版视频，Seedance2.0fast模式，全能参考
**镜头语言**：微距特写镜头，缓慢推进
**核心主体**：@Image {product_image}的[膏体/液体]，涂抹在[皮肤/头发/牙齿]上
**商业场景**：微观层面，展示[细胞修复/污垢清除/色素分解]的过程
**强制约束**：画面带有[治愈感/解压感/科技感]，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
**基础参数**：10-15s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：固定镜头，中景展示专家
**核心主体**：@Image {product_image}的产品，专业的[皮肤科医生/营养师/健康专家]推荐
**商业场景**：[诊所/实验室/专业工作室]，[医疗仪器/实验设备/专业证书墙]
**强制约束**：专家穿着白大褂，表情严肃专业，口型与@Audio同步，语气权威可信，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
**基础参数**：8-10s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：时间流逝效果，快速生长/变化
**核心主体**：@Image {product_image}的使用效果，从[种子/稀疏状态]到[盛开的花朵/浓密头发/理想状态]
**商业场景**：自然生长过程，[浇水/使用产品]
**强制约束**：整个过程自然流畅，最后定格在完美的效果展示，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：8-12s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：固定镜头展示主播，切换镜头到街头采访/产品展示
**核心主体**：@Image {product_image}的产品，新闻主播报道[某产品引发抢购/发现革命性成分/限时优惠即将结束]
**商业场景**：美国电视新闻突发报道场景，屏幕下方滚动字幕"Breaking News"
**强制约束**：整体风格逼真，带有新闻特有的紧迫感，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：8-10s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：完全复刻@Video的探镜运镜轨迹，极度贴近产品表面
**核心主体**：@Image {product_image}的产品，[穿过戒指环/掠过表盘/滑过项链链条]
**商业场景**：展示[金属的光泽/镶嵌的细节/材质的纹理]，背景虚化
**强制约束**：画面带有[高级的电影质感/微距景深/流畅的运镜节奏]，突出产品精致感，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：10-15s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：FPV第一人称视角，手持GoPro拍摄，镜头随着[身体晃动/动作移动]
**核心主体**：@Image {product_image}的产品，[拆开包装/组装产品/开始使用]
**商业场景**：沉浸式体验，体现出[极致的快乐/自由的体验/沉浸的享受]
**强制约束**：画面带有[轻微的运动抖动/真实的呼吸感]，仿佛观众亲自在体验，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：8-12s竖版视频，Seedance2.0fast模式，全能参考
**镜头语言**：参考@Video的网红风格和拍摄节奏
**核心主体**：@Image {product_image}的产品，博主表情[夸张惊喜/疯狂点头/极力推荐]
**商业场景**：真实的用户分享，语气[兴奋激动/真诚分享/幽默风趣]
**强制约束**：画面带有[真实的UGC质感/轻微的手持抖动/自然的灯光]，表现出对产品的[极度喜爱/强烈推荐/惊喜发现]，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
...
<prompt_template>
**基础参数**：8s竖版视频，Seedance2.0模式，全能参考
**镜头语言**：按时间区间运镜，确保每个时间段的运镜和画面内容准确对应
**核心主体**：@Image {product_image}的产品，按分镜时间线展示
**商业场景**：0-2s{shot_1_content}, 2-4s{shot_2_content}, 4-6s{shot_3_content}, 6-8s{shot_4_content}
**强制约束**：过渡自然流畅，即梦SeeDance适配，画面无模糊，无畸变，音画同步
</prompt_template>
<reference_type>@Image（产品图）</reference_type>
...
2.旁白：年轻美式英语口语化发音，语气带假装惋惜又兴奋的感觉，台词完全对应文字："im SO sorry to those that got the LEGO Hotel Model, 6 Anglcus it just went on a mega triple discount with shipping covered!"
